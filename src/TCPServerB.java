...
public class TCPServerB implements  Runnable
{
	...
	public static void main(String argv[]) throws Exception
	{
		...
		outToServer.writeBytes("join(serverB)"+ "\n");
		welcomeSocket=new ServerSocket(6001);
			 ...
			 public void run()
			 {
				 ...
				 { 
					 ...
					 if(inFromA.length()>0)
					 {
						 ...
						 System.out.println("ServerA: " + inFromA + '\n' );

									if(inFromA.contains("Chat(")){
										
										
											StringTokenizer st = new StringTokenizer(inFromA, ",");
									...
									else if(inFromA.contains("join(") && !inFromA.contains("serverB")){
										
										String x = inFromA.substring(5);
										Allmembers.add(x);
										
										for(int i=0; i<members.size();i++){
											if(members.get(i).equals("serverC")){
												try {
									...
									else if(inFromA.contains("RE:")){
										String x = inFromA.substring(3);
										...
										for(int i=0; i<members.size();i++){
											if(members.get(i).equals("serverC")){
												try {
													OutputStream os = (sockets.get(i)).getOutputStream();
													DataOutputStream outToOtherClient = new DataOutputStream(os);
													outToOtherClient.writeBytes(inFromA+"\n");
												} catch (IOException e) {
													// TODO Auto-generated catch block
													e.printStackTrace();
												}
		 ...
		 while(true)
		 {
			 ...
			 Thread t1=new Thread(new TCPServerB(connectionSocket));
